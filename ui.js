...
const S = {
  ...
  chat: {
    ...
    candidates: null,
    messages: [
      { role: 'bot', text: '안녕하세요! AI 결제 큐레이터입니다.\n- ☕ **\'스타벅스 2만원 결제할 건데 좋은 카드 찾아줘\'** (혜택액 자동 계산)\n- ✈️ **\'해외여행 갈 때 쓰기 좋은 카드 추천해줘\'** (상황별 맞춤 추천)\n- 💳 **\'삼성카드랑 현대카드 혜택 비교해줘\'** (스펙 비교)\n\n궁금한 점을 위와 같이 편하게 물어보세요!', html: null }
    ]
  }
};
...
function bind() {
  ...
  if (catAllChk) {
    catAllChk.addEventListener('change', e => {
      const bi = $('#brandInput');
      if (e.target.checked) {
        S.q.brand = '';
        if (bi) { bi.value = ''; bi.disabled = true; }
      } else if (bi) {
        bi.disabled = false;
        bi.focus();
      }
  ...
  $$('[data-chip]').forEach(el => el.addEventListener('click', () => {
    ...
    const catAllRow = $('#catAllRow');
    const catAllTxt = $('#catAllTxt');
    const chk = $('#catAllChk');
