...

{
  ...
  "benefits": [
    {
      "benefit_name": "혜택명",
      "category": "다음 목록 중 정확히 하나만 선택 (목록 밖 값이나 여러 개를 |로 이어붙이는 것 금지, 가장 가까운 것 하나만 고를 것): 외식 | 영화 | 통신 | 교통 | 대형마트 | 쇼핑 | 의료 | 도서 | 뷰티 | 테마파크 | 여행 | 문화 | 기타",
      "merchant_scope_type": "payment_method | merchant | mixed 중 하나 또는 null",
      "merchants_or_scope": "적용 가맹점/범위 (가급적 널리 쓰이는 대표 브랜드명으로 정규화해서 추출하되(예: '아웃백스테이크하우스' -> '아웃백'), 확신이 서지 않더라도 원문 대신 일단 대표명으로 추측해서 적어주세요. 나중에 관리자가 별도로 검토할 것입니다. 쉼표(,) 혹은 | 기호로 구분)",
      "merchant_normalization_flags": [
        {
          "original": "문서에 적혀 있던 원본 가맹점명 (예: 아웃백스테이크하우스)",
          "normalized": "merchants_or_scope에 적은 정규화된 대표 브랜드명 (예: 아웃백)",
          "category": "업종 분류 (외식, 카페/베이커리, 쇼핑, 배달, 마트/편의점, 교통, 주유/차량, 통신/공과금, 의료, 여행/숙박, 뷰티/미용, 교육, 엔터/문화, 기타 중 택1)",
          "confidence": "high | medium | low (정규화가 널리 쓰이는 확실한 명칭인지 여부)"
        }
      ],
      ...
      "required_grade": "통신사 멤버십처럼 등급별로(VIP/GOLD/SILVER 등) 혜택이 달라지는 경우 이 혜택이 요구하는 최소 등급명, 등급 구분이 없으면 null",
  ...
  "merchant_normalization_flags": [
    {
      "original": "문서 내 원문 가맹점명",
      "normalized": "AI가 추정한 정규화 대표명 (확신이 없더라도 가장 그럴듯한 이름 제안)",
      "confidence": "high | medium | low 중 문서에서 이 가맹점을 대표명으로 치환하는 것에 대한 확신도 (자신 없다면 low)"
    }
  ]
}

benefits/rules 배열은 문서에서 실제로 확인 가능한 항목만 포함하고, 근거 없는 값은 만들어내지 마세요.
merchant_normalization_flags 배열에는 문서에서 찾은 가맹점 명칭을 어떻게 정규화했는지 전부 남겨주세요.`;
...
${parsedText.slice(0, 60000)}`;
    ...
    try {
      const res = await fetch(CHAT_URL, {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.1 * (attempt - 1), // 실패할수록 온도를 약간 높여 다른 결과를 유도
          max_tokens: 16000,
        }),
      });
      ...
      return parsed; // 성공 시 즉시 반환
    } catch (err) {
      console.warn(`[AI 추출 실패] 시도 ${attempt}/${MAX_RETRIES}: ${err.message}`);
      lastError = err;
  ...
  // 모든 재시도 실패 시 에러 던지기
  throw new Error(`AI 데이터 추출 최종 실패 (총 ${MAX_RETRIES}회 시도): ${lastError.message}`);
}
